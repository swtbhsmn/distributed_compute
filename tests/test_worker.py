...
from distributed_compute.models import Benchmark
from distributed_compute.worker import load_or_create_state


def test_worker_identity_and_benchmark_are_persistent(tmp_path: Path, monkeypatch) -> None:
    state_path = tmp_path / "worker.json"
    benchmark = Benchmark(elapsed_seconds=0.5, operations=100, operations_per_second=200)
